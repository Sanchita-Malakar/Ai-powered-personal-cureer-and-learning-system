...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use signup or signUp method from Supabase auth
      const signUpFn = (supabase.auth as any).signup || supabase.auth.signUp;
      const { data, error: signUpError } = await signUpFn.call(supabase.auth, {
        email: email.trim(),
        password,
        options: {
          emailRedirectTo,
    ...
    <div className="min-h-screen bg-canvas bg-ambient-mesh flex flex-col justify-between py-6 px-4 sm:px-6 lg:px-8">
      ...
      <main className="max-w-md w-full mx-auto my-auto">
        <div className="bg-surface/90 backdrop-blur-xl border border-border/90 rounded-3xl p-7 sm:p-9 shadow-card-3d relative overflow-hidden">
          {/* Subtle Ambient Glow */}
          <div className="absolute -top-16 -right-16 w-44 h-44 bg-ai/15 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="mb-6 relative z-10">
            ...
            <p className="text-xs sm:text-sm text-ink-muted mt-1">
              Enter your student email and a secure password to get started.
            </p>
          </div>
          ...
          {/* Email confirmation message when data.session is null */}
          {message && (
            <div className="mt-3.5 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/25 text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm font-medium flex items-center justify-center gap-2 animate-in fade-in">
